...
if __name__ == "__main__":
    ...
    os.system("twarc/twarc.py --consumer_key "+CONSUMER_KEY+" --consumer_secret "+CONSUMER_SECRET+" --access_token "+token+" --access_token_secret "+secret+" --search railsconf > tweets-day5.json")
    
    # t = Twarc(CONSUMER_KEY, CONSUMER_SECRET, token, secret)
    # opt = open("tweets_day_1.txt", "w")
    # for tweet in t.search("railsconf", {"mindate": "1-may-2014"}):
    #     try:
    #         opt.write(tweet["text"].decode('utf8')+"\n")
    #     except Exception:
    #         pass
    # opt.close()