...
if __name__ == '__main__':
    files_in_dir = os.listdir('sitesq')
    os.chdir('sitesq')
    for file_in_dir in files_in_dir:
        ...
        for word in words:
            ...
            word = word.strip()        
            print word
            word_in_db = db.boilerpipe_word_count.find_one({"word" : word})
            if word != "":
                if word_in_db is None:
                    db.boilerpipe_word_count.insert({"word" : word , "count" : 1})
                else:
                    db.boilerpipe_word_count.update({"word" : word}, {"word" : word, "count" : word_in_db["count"]+1})